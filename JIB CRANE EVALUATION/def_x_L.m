...
E=205*(10^3); %MPa modulo elastico acciaio;
phi_AD=0;  %angolo di deformazione della colonna AD;
A_tie_r=((30^2)*3.14156)/4; %mm^2 RICORDATI DI CAMBIARE LE DIMENSIONI DEL TIE ROD 
pos_amm=[];
x_L_mm=x_L*1000; % mm
...
for z=2:length(x_pend)
    for j=1:length(x_L)
        phi_AD=(1000*Md_mat(z,j)*h*1000)/(E*I_col_zz);
        ...
        if x_pend(z)>x_L(j)
            ...
            if x_L(j)<(x_pend(z))/2
                v_AB_w=((w*(x_pend_mm(z))^3)/(24*E*I_beam_zz))*x_L_mm(j);
            elseif x_L(j)>(x_pend(z))/2
                v_AB_w=((w*(x_pend_mm(z))^3)/(24*E*I_beam_zz))*(x_pend_mm(z)-x_L_mm(j));
            end
            
        elseif x_pend(z)<x_L(j)
            v_BE_w=((w*(l_tot_mm-x_pend_mm(z))^3)/(6*E*I_beam_zz))*(x_L_mm(j)-x_pend_mm(z));
