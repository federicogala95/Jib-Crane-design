...
for k=1:length(pos_analysis_stress_c)
     ...
     sigma_x_col_90(k)=(1000*M_col(23,k,v)*(-b))/(I_col_xx)+(N_col_b(23,k,v)*(h_sez/2)*(h_sez/2+(2*I_col_zz/(2*A_beam*h_sez))))/I_col_zz+(N_col(23,k,v)/(2*A_beam));
     %tau_col_shear_90(k)=V_col(23,k,v)*((h_i/2)^2)/I_col_xx+V_col(23,k,v)*(b^2/8)/I_col_xx+V_col(23,k,v)*((b^2)/8)/I_col_xx; 
     tau_col_shear_90(k)=0;
     ...
     sigma_eq_col_90(k)=((sigma_x_col_90(k)^2)+4*((tau_col_shear_90(k)+tau_col_torque_90(k))^2))^(1/2);
end
