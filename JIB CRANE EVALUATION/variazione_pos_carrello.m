...
w=30.7*9.81; %N/m IPE A 170  da catalogo a cui sono associati anche E ed I insieme alle dimensioni degli spessori;
max_V=[];
max_M=[];
l_tot=3.1; %metri--> lunghezza della trave orizzontale;
...
x=0:0.1:3.1; %metri--> numero di discretizzazioni che si è deciso di adottare per analizzare la trave;
...
for z=1:length(x_pend) %diversamente da prima consideriamo le possibili posizioni del carrello;
    for i=1:length(x_L)
         ...
         Rd_y=Ra_y+Rb*sin(alpha_1)+p__c*3.5;
...
%impiegati per evitare di avere incongruenze legate alla dimensione;
     ...
     %posizione come per la trave orizzontale ma cambiano i moduli con i e z;
      ...
      N1_col_w=-y_rev(1:26)*p__c; %applicazione baricentrica --> NON GENERA MOMENTO FLETTENTE
      N2_col_w=-y_rev(27:end)*p__c;
     ...
     N_beam_post=zeros(1,(32-length(x_pend)));
...
x=0:0.1:3.1; %Ridefiniamo i due vettori x e x_L perchè erano stati eliminati durante l'iterazione
x_L=0.6:0.1:3;
...
for z=2:length(x_pend) %RICORDATI CHE NELLA POSIZIONE (1,1) SI OTTENGONO VALORI STRANI DAL MOMENTO CHE Rb TENDE AD INF. --> escludine i risultati
    ...
    M_sum_var_L=reshape(M_sum_var_L,1,length(x_L)*length(x));
    V_sum_var_L=reshape(V_sum_var_L,1,length(x_L)*length(x));
